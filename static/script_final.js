...
input.setSelectionRange(1000000000, 1000000000);
}
...
function removeSearch(current_path, input){
    start = current_path.indexOf(input);
    temp_end = current_path.slice(start).indexOf("&");
    end = current_path.length - current_path.slice(start).length + temp_end;
    remove = current_path.slice(start, end + 1);
    current_path = current_path.replace(remove, "");
...
function searchFunction() {
    ...
    if(input != "") {
        current_path = current_path.replace(/searchall=|searchtitle=|searchdescription=|searchcode=/g, "search=");
        var indexOfFirstQuestionMark = current_path.indexOf("?");
        var indexOfFirstAmpersand = current_path.indexOf("&");
        var indexOfFirstEqual = current_path.indexOf("=");
        var new_path;
        var select = document.getElementById("searchlimit");
        var searchlimit = select.value;
        tailpath = current_path.slice(indexOfFirstQuestionMark + 1);
        if(current_path.includes("search=") === false) {
            if(current_path.includes("&") === true) {
                new_path = current_path.slice(0, indexOfFirstQuestionMark + 1) + "search=" + input + "&" + tailpath;
                new_path = new_path.replace(/search=/, "search" + searchlimit + "=");
            } else {
                current_path = current_path.replace(window.location.href, "");
                new_path = current_path.replace("?", "") + "?search=" + input + "&";
                new_path = new_path.replace(/search=/, "search" + searchlimit + "=");
            }
        } else {
            new_path = current_path.slice(0, indexOfFirstEqual + 1) + input + current_path.slice(indexOfFirstAmpersand);
            new_path = new_path.replace(/search=/, "search" + searchlimit + "=");
        }
        document.location.href = new_path;
    } 
        ...
        var indexOfFirstQuestionMark = current_path.indexOf("?");
        var indexOfFirstAmpersand = current_path.indexOf("&");
        var remove = current_path.slice(indexOfFirstQuestionMark + 1, indexOfFirstAmpersand + 1);
        if(current_path.includes("&") === true) {
            new_path = current_path.replace(remove, "");
            new_path = new_path.replace(/search=/, "search" + searchlimit + "=");
            document.location.href = new_path;
...
function searchFunctionEnter(event) {
    if (event.key !== 'Enter'){
        return;
    }
    searchFunction();
...
function checkBlankInput(event){
    current_path = window.location.href;
    var matches = current_path.match(/searchall=|searchtitle=|searchdescription=|searchcode=/g);
    if (current_path.includes("page=")){
        current_path = removePage(current_path);
    }
    if (document.getElementById("search-box").value == '' && matches !== null){
        current_path = removeSearch(current_path, matches[0]);
        document.location.href = current_path;
    }
};
...
current_path = window.location.href;
if (current_path.includes("school") == false) {
...
function displayMyCourses() {
    var x = document.getElementById("myCourses");
    if (x.style.display === "none") {
        x.style.display = "block";
    } else {
        x.style.display = "none";
    ...
    for (var i = 0 ; i < cookies_list.length; i++) {
        if(cookies_list[i].includes("_ga") == false && cookies_list[i].includes("darkmode") == false){
            var y = cookies_list[i].replace("=on","").replace(" ", '')
            res += "<li><a href=" + '/' + y + ' ' + 'target="_blank">' + y + "</a></li>"
        }
    }   
    document.getElementById('mycourses').innerHTML = res
    return;    
}	

function removeCookies(code){
    s = document.getElementById('remove' + code);
    s.remove();
}
...
function addtoPlanner(code) {
    ...
    var element = document.getElementById('planner' + code);
    if(planner == ''){
